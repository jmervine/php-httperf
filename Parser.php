 ...
 * @version 1.0.0
 *
 * Parser class for httperf raw output.
 */
...
class Parser {
   ...
   * Parse method
   *
   * @param string||string[] $raw String or array containing raw output from httperf.
   *
   * @return mixed[] Returns array containing parsed results from httperf output.
   */
  public static function parse($raw) {
    ...
    foreach ($raw as $line) {
      ...
      preg_match(self::verboseExpression(), $line, $match);
      if (isset($match[1])) {
    ...
    if (!empty($verbose_connection_times)) {
      ...
      foreach (self::percentiles() as $percentile) {
        $matches["connection_time_".$percentile."_pct"] = self::calculatePercentiles($percentile, $verbose_connection_times);
      }
  ...
  /**
   * @return string Returns string with 'Connection lifetime' matcher RegExp
   */
  private static function verboseExpression() {
    return "/^Connection lifetime = ([0-9]+\.[0-9]+)(\s?)/";
  }

  /**
   * @return string[] Returns array of strings with needed matcher RegExp for parsing
   * raw httperf output.
   */
  ...
  /**
   * @return int[] Returns array of ints to be used in generating percentiles.
   */
   ...
   * @param int[] $value The array of connection times to be used in the
   * percentile calculation.
   *
   * @return int Returns int of the percentile calculation.
   */
  private static function calculatePercentiles($percentile, $values) {
    $len = count($values);
